...
class CollectLinks:
    ...
    def exec(self, seed_url: Url, depth: int):
        logger.info(f'seed_url: {seed_url}, depth: {depth}')
        current_depth = 0
        ...
        while current_depth_urls and current_depth < depth:
            ...
            for url in current_depth_urls:
                if seed_url.domain != url.domain:
                    logger.info(f'Ignore domain which is different from the seed url domain. url: {url}')
                    continue

                logger.info(f'Crawl web page. target url: {url}')
